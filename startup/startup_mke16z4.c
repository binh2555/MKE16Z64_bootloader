...
#define BOOT_BUUTON_PORT            (GPIO_D)
#define BOOT_BUUTON_PIN             (2U)       /* user pin 2 of port D(switch 3 in kit mke16z64) as boot button */
...
// #endif
/*******************************************************************
 * config to jump to bootloader app or user app
 ********************************************************************/
    Driver_EnableGPIODClock();
    Driver_Mux(BOOT_BUUTON_PORT, BOOT_BUUTON_PIN, MUX_GPIO_MODE);
    Driver_SetPinMode(BOOT_BUUTON_PORT, BOOT_BUUTON_PIN, INPUT_MODE);

    if (0 == Driver_ReadDataInput(BOOT_BUUTON_PORT, BOOT_BUUTON_PIN))
    {
        bootloader_main();
        //user_main();
    }
